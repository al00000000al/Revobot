...
zip -r $ZIP_NAME . -x ".git/*" "build/*" "*.zip" ".github*" "tmp/*" "vendor/*"
...
ssh -i $SSH_KEY -p $SSH_PORT $SSH_USER@$SSH_HOST "cd $SSH_DIR; unzip -o $ZIP_NAME; rm $ZIP_NAME; php generation.php; cd build; ./compile.sh; sudo ./restart.sh"
