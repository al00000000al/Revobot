...
class PassCmd extends BaseCmd
{
    ...
    {
        $from_coins = (int)PMC::get(self::getMoneyKey($provider, $old_user_id));
        $to_coins = (int)PMC::get(self::getMoneyKey($provider, $user_id));
        $result = $to_coins + $from_coins;
        PMC::set(self::getMoneyKey($provider, $user_id), $result);
        PMC::set(self::getMoneyKey($provider, $old_user_id), 0);
    }
